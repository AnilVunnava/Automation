...
public class HMY_TimeEntryTests extends AbstractSmokeTests {
	 ...
	 * @return WebDriver upon the execution
	 * 
	 *         <pre>
	 * {@code
	 * Employee Loginto Harmony WTE
	 * }
	 * 
	 *         <pre>
	 */
	 ...
	 * @param driver
	 *            - WebDriver driver
	 * @return Map<String, FluentWebElement> upon the execution contains all
	 *         Header Links
	 * 
	 *         <pre>
	 * {@code
	 * This gets all the Header Links in Harmony like
	 * 	1) Profile
	 * 	2) Dashboard etc..
	 * }
	 * 
	 ...
	 *            - double index
	 * @param phrase
	 *            - String phrase (Either work/break)
	 * @return String upon the execution returns either the work/break values
	 * 
	 *         <pre>
	 * {@code
	 * Used to Calculate and increment the InOut values and Breaks values for an InOut user
	 * }
	 * 
	 *         <pre>
	 */
	 ...
	 * @param payPeriod
	 *            - String payPeriod
	 * @return String upon the execution PayPeriod in Harmony Specific format
	 * 
	 *         <pre>
	 * {@code
	 * This gets PayPeriod in Harmony Specific format
	 * }
	 * 
	 *         <pre>
	 */
	 ...
	 *            - Input Map Values separated by '|'
	 * @param WebDriver
	 *            - Driver at the current session
	 * 
	 *            <pre>
	 * {@code
	 * Employee Loginto Harmony (WTE) if for the first time this is used to accept the Terms&Conditions
	 * }
	 * 
	 *            <pre>
	 */
	 ...
	 * @param WebDriver
	 *            - Driver at the current session
	 * @return Boolean upon the execution returns the status either true/false
	 * 
	 *         <pre>
	 *  {@code
	 *  Employee Loginto Harmony (WTE) and executes all the scenarios below.
	 *  		1) Add Work
	 * 		2) Add Break
	 * 		3) Add Project hours
	 * 		4) Helper Class Test
	 ...
	 * 	 	8) Submit
	 * 		9) Submit All
	 * 		10) Finally Approval From TMC
	 * 		11) UDF Value/Variable Check
	 * 		12) Add Assignment
	 * }
	 * 
	 * {@code
	 *  And below are some end-to-end scenarios covered too.
	 * 		1)	I/O - Enter a comment in Harmony and Verify in TMC
	 * 		2)	HDP - Enter a comment in Harmony and Verify in TMC
	 * 		3)	UDF - Enter a comment in Harmony and Verify in TMC 
	 * 		4)	I/O - Add attachment to TS in Harmony and Verify in TMC
	 * 		5)	HDP - Add attachment to TS in Harmony and Verify in TMC
	 * 		6)	UDF - Add attachment to TS in Harmony and Verify in TMC
	 * 		7)	Add assignment from zero
	 * 		8)	Add worked hours in I/O with break in Harmony  and submit in TMC and verify 
	 * 		9)	Add assignment in Harmony and verify in TMC
	 * 		10)	OT Override - I/O -Submit time in Harmony with OT and verify in TMC
	 * 		11)	OT Override - HPD -Submit time in Harmony with OT and verify in TMC
	 * 		12)	OT Override - Monthly -Submit time in Harmony with OT and verify in TMC
	 * 		13)	UDF - Save time in Harmony with UDF, verify and submit  in TMC
	 * 		14)	UDF - Submit time in Harmony with UDF, verify and modify  in TMC via Edit UDF option
	 * 		15)	Projects - Add project and submit time in harmony. Verify in TMC and edit  in TMC via Edit Projects option
	 * 
	 *  And In all these scenarios I did Time Card submission the two ways "Submit and Submit All" from Harmony.
	 *
	 * }
	 * 
	 *         <pre>
	...
	public boolean TimeEntryTest(final SuiteConfig data,
		...
		try {
			...
			for (FluentWebElement time : weekTimesheet) {
				...
				if (status.equals("No Time Entered")) {
					if (actualMonth.startsWith(currentMonth)) {
						...
						if (inputValues.containsKey("deleteAssignments")) {
							if (Boolean.valueOf(inputValues
									.get("deleteAssignments"))) {
								FluentWebElements mulAssignmts = fluent.divs(By
										.id("siteassignment"));
								for (FluentWebElement multiAssign : mulAssignmts) {
									if (multiAssign.isEnabled().value()
											&& multiAssign.isDisplayed()
													.value()) {
										FluentWebElement element = multiAssign
												.div(By.className("siteassignmentheader"));
										if (element.isEnabled().value()
												&& element.isDisplayed()
														.value()) {
											String assignVal = element
													.getText().toString();
											if (assignVal
													.equalsIgnoreCase(inputValues
															.get("assigName"))) {
												FluentWebElement rmvAssign = element
														.span(By.className("enableworkstatus"));
												if (rmvAssign.isEnabled()
														.value()
														&& rmvAssign
																.isDisplayed()
																.value()) {
													rmvAssign.click();
													Thread.sleep(wait);
													FluentWebElement yes = fluent
															.button(By
																	.xpath("contains(.,'Yes')"));
													if (yes.isEnabled().value()
															&& yes.isDisplayed()
																	.value()) {
														yes.click();
														Thread.sleep(wait);
							...
							if (isMonthly) {
								FluentWebElement siteHeader = fluent
										.div(By.className("dayentrybackgrounddefault"));
								String totalHrs = siteHeader
										.div(By.className("totalhours"))
										.getText().toString();
								log.info("Time Total : " + totalHrs);
								log.info("Inner Status : "
										+ siteHeader
												.div(By.className("status"))
												.getText().toString());
								takeScreenShots("/" + data.getScript()
										+ "/TimeEntry/screenshot",
										data.getGroup(), driver);
			...
			log.info("Checking for Show OT, Approve/Delete from TMC for Employee of Type : "
					+ inputValues.get("empType"));
			if (!inputValues.containsKey("checkEntryRounding")) {
				Thread.sleep(wait);
	 ...
	 *            <pre>
	 * {@code
	 * This Re-Checks the time status either Approved/Rejected/Saved not Submitted etc... in Harmony for an Employee after the process done via TMC.
	 * This will be called on the input parameter checkTMC=true
	 * }
	 * 
	 *            <pre>
	 */
	 ...
	 *            <pre>
	 * {@code
	 * This either Approves/Deletes or does both after approving via TMC for an Employee who entered time from HMY.
	 * This will be called on the input parameter checkTMC=true
	 * }
	 * 
	 *            <pre>
	 ...
	 *            <pre>
	 * {@code
	 * This will change the state of TimeCard to NoTimeEntered from PendingApproval state for an Employee who entered time from HMY and did a Submit.
	 * This will be called on the input parameters approveFromTMC=false and deleteFromTMC=false
	 * }
	 * 
	 *            <pre>
	 */
	 ...
	 *            <pre>
	 * {@code
	 * This is actual clear method which will see the submitted comments and attachments download if any before clearing Time
	 * }
	 * 
	 *            <pre>
	 */
	 ...
	 *            <pre>
	 * {@code
	 * Evaluates the DashBoard had all the required links and checks their Navigation
	 * }
	 * 
	 *            <pre>
	 */
	 ...
	 *            - SuiteConfig data
	 * @param driver
	 *            - WebDriver driver at current session
	 * @param inputValues
	 *            - Map<String, String> inputValues
	 * 
	 *            <pre>
	 * {@code
	 * The actual method where attachments and Comments were added to a TimeCard
	 * }
	 * 
	 *            <pre>
	 */
						...
						if (data.getScript().equals("HMY_Validate_Dashboard")) {
							...
							if (Boolean.valueOf(inputValues.get("addEmployee"))) {
								...
								Map<String, Object> state = AddEmployee(data,
										inputValues);
								if (!state.isEmpty()) {
									if (Boolean.valueOf((boolean) state
											.get("status")))
										if (Boolean.valueOf(inputValues
												.get("processEmployee"))) {
							...
							EmployeeData employee = DatabaseFactory
									.getEmployeeForClient(
											...
											inputValues.get("client"),
											inputValues);
							inputValues.put("userId", employee.getEmpEmail());
							inputValues.put("password", employee.getPassword());
							inputValues.put("recordId", employee.getRecordID());
							log.info("Input Values : " + inputValues);
							status = TimeEntryTest(data, inputValues, null);
						}
